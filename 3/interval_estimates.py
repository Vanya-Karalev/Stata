...
def exponential_check(inp_data: list, meaning_level: float = 0.01):
    ...
    mean = get_mean(inp_data) + 0.009
    sq_dev = get_square_root_deviation(inp_data)
    exponential_lambda = 1 / mean

    exp_values = [F_expon(exponential_lambda, data[1][i]) - F_expon(exponential_lambda, data[0][i]) for i in range(len(data[0]))]
    ...
    check = [data[2][i] / data[2][i-1] for i in range(1, 36)]
    print(check)
